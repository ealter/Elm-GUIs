...
Because \texttt{hoverablesJoin} was applied throughout the entire tree, it
...
specified width and height (\texttt{spacer : Int -\textgreater{} Int
-\textgreater{} Element}). For our purposes, the spacer has the same width as
...
spacer based on hover information. The spacer helped to align the submenu with
...
A submenu is rendered to the screen if one of three conditions
...
Applying \texttt{extend} makes a \texttt{Signal Bool} wait a millisecond to
transition from true to false. Unlike a delay, false-to-true transitions still
occur immediately. By applying this function to every hovering Boolean in the
Menu structure, we give the mouse time to move from a menu Element to its
submenu before that submenu disappears. A millisecond was all that was needed.
...
\texttt{|>}, but it can be read as a pipeline.)
