...
A typical use of \texttt{hoverables} is as follows. An initial
\texttt{a} is supplied that becomes the default value of
\texttt{events}. If \texttt{hoverable} is never applied, \texttt{events}
is constant. If it is, the \texttt{Element} argument of
\texttt{hoverable} in the record acts much like it does in
\texttt{hoverable} the top-level combinator: provides the static visual
appearance of the result \texttt{Element}, which also has hover
detection added. Hover events are boolean, so the function argument is
called on every such event to produce an \texttt{a} that becomes an
event on \texttt{events}.

What gives \texttt{hoverables} its plural name is that the record's
\texttt{hoverable} may be applied multiple times so that multiple
elements report their hover information on \texttt{events}. The
polymorphic \texttt{a} type exists to uniquely identify each element,
though it is also common for the \texttt{a} to also include the boolean
event. The simplest choice of \texttt{a} is simply \texttt{Bool}, and
this allows \texttt{hoverables} to implement \texttt{hoverable}:
...
The \texttt{a -> Bool} function trivially becomes the identity. With a simple
change, we can create a function that acts on \texttt{Signal Element} instead of
\texttt{Element}:
