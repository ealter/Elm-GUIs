...
technique. For labeled buttons, press events are represented by unit or
...
convenient. If the menus were dynamic, it would be useful to determine
when an event occurred because the user makes a selection vs.~when the
menu updated and the previously selected string was no longer available.

% These are likely unnecessary and cause page breaks in bad places.
%
%\texttt{Graphics.Input} does not currently support radio buttons.
%
%As an aside, \texttt{button} is an impure function. One can create two
%buttons with the same label and place them in different places on the
%screen (or place one not at all), and the signals of press events will
%be different. Lifting a joined version onto a signal of strings does not
%change this fact.
